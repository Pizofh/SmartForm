...
//CONYUGUE
    ...
    await this.DatosFamiliaresTab.click();
    await this.nombre_conyugue.fill('WFER#""#$');
    await this.cedula_conyugue.fill('1');
    await this.profesion_oficio_conyugue.fill('FGRT%&/');
    await this.celular_conyugue.fill('2');

//DATOS PADRE
    await this.nombre_padre.fill('FRGTRHY&%$#');
    await this.identificación_padre.fill('1');
    await this.telefono_padre.fill('1');
    await this.oficio_profesion_padre.fill('#"$RETGRER#$"');


  //DATOS MADRE
    await this.nombre_madre.fill('RGTRHUYJ&/%');
    await this.identificación_madre.fill('1999999999999999999999999999999999999999');
    await this.telefono_madre.fill('1');
    await this.oficio_profesion_madre.fill('FGRT%$#"QQ#"');


  //DATOS HIJOS
    await this.nombre.fill('EDWRFGT$%#"$%');
    await this.edad.fill('999999999');
    await this.identificacion.fill('1');


  //DATOS HERMANOS

    await this.primer_apellido.fill('&%$RTGGBFF');
    await this.segundo_apellido.fill('#"$%T#$RFEGRVF');
    await this.primer_nombre.fill('#"$%$#ERFVG');
    await this.segundo_nombre.fill('QEDWEFVRGT%#$"$#"#')
    await this.identificacion_hermano.fill('25');
    await this.ocupacion_hermano.fill('E$#%&/(/UTYGFe');
    await this.celular_hermano.fill('3139946548222222222222');
